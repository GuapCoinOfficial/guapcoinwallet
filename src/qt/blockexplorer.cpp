...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::GUAP, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (guapcoin.conf).");
            QMessageBox::warning(this, "Guapcoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#8f802d; bgcolor:#fafafa;}\n a, span { font-family: monospace; }\n span.addr {color:#8f802d; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #fafafa;}\n td.d0 {font-weight: bold; color:#8f802d;}\n h2, h3 { white-space:nowrap; color:#8f802d;}\n a { color:#337ab7; text-decoration:none; }\n a.nav {color:#337ab7;}\n";
