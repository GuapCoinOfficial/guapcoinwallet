...
*Setup instructions for a gitian build of Guapcoin using a Debian VM or physical system.*
...
- [Building Guapcoin](#building-guapcoin)
...
Clone the git repositories for guapcoin and gitian and then checkout the guapcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the guapcoin repository
...
To build Guapcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the guapcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/guapcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/guapcoin/guapcoin
...
For example:
```bash
URL=https://github.com/konez2k/guapcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit guapcoin=${COMMIT} --url guapcoin=${URL} ../guapcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit guapcoin=${COMMIT} --url guapcoin=${URL} ../guapcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit guapcoin=${COMMIT} --url guapcoin=${URL} ../guapcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/guapcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/guapcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/guapcoin-build.assert
```
...
[guapcoin/gitian.sigs](https://github.com/guapcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
