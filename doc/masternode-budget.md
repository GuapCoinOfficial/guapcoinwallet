...
Guapcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<guapcoin_address\> \<monthly_payment_guapcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ GUAP, _12_ times over the course of a year totaling _24000_ GUAP.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<guapcoin_address\> \<monthly_payment_guapcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` GUAP.
 ...
 * vote-many          - Vote on a Guapcoin initiative
 * vote-alias         - Vote on a Guapcoin initiative
 * vote               - Vote on a Guapcoin initiative/budget
